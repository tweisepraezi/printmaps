...
- v1.0.0 - 2026/03/28 : revised, compiled with go go1.26.1
...
func ReadMetadata(pmData *PrintmapsData, id string) error {
	...
	if err != nil {
		log.Printf("error <%v> at os.ReadDir(), path = <%v>", err, path)
		return err
	}
	...
	for _, fileInfo := range files {
		if fileInfo.IsDir() == false {
			if !(fileInfo.Name() == FileMetadata || fileInfo.Name() == FileMapstate || fileInfo.Name() == FileMapfile) {
				// Info() aufrufen, um das os.FileInfo-Objekt zu erhalten
				if info, err := fileInfo.Info(); err == nil {
					fileList = fileList + fmt.Sprintf(format, fileInfo.Name(), info.Size())
					format = ",%s,%d"
				}
			}
		}
